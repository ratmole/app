    ...
    draw: function(element,series) {
        ...
        for (s in series)
        {
            ...
            for (z in data)
            {
                ...
                ctx.fillStyle = "#0699fa";
                if (dayid==0) ctx.fillStyle = "#077900";
                
                if (day==1) ctx.fillStyle = "#aaa";
		...
		if ((plot_height-y)>20) { 
                    ...
                    ctx.fillText((data[z][1]*scale).toFixed(1),x,y-10);

                    ctx.fillStyle = "#ccccff";
                    ctx.font = "12px arial";
                    ctx.fillText(day,x,plot_height-25);
                    ctx.fillText(days[dayid],x,plot_height-10);
		} else {
		    ctx.textAlign    = "center";
		    ctx.fillStyle = "#333";
		    ctx.fillStyle = "#ccccff";
